...
$data = fread($fp,filesize($file));
echo $data; 
?>/smarthome5/js/Highcharts-7.0.3/code/highcharts.js"></script>
...
?>/smarthome5/table/" role="tab" aria-controls="v-pills-profile" aria-selected="false"><i class="fas fa-table" style='font-size:17px'></i> Table</a>
      <a class="nav-link active font-weight-bold mb-1" id="v-pills-messages-tab" href="http://<?php
//To Read File
$file = 'C:\xampp\htdocs\ip.txt';
$fp = fopen($file, 'r');
$data = fread($fp,filesize($file));
...
<!---->
<!---->
<h3 class="text-white ml-4">Menampilkan Statistik Sensor</h3>

<!-- awal grafik suhu-->
<div id="container" class="rounded border border-secondary ml-4 mr-4 mt-4" style="max-width:90%; max-height:400px;margin-bottom: 30px;"></div>
<script type="text/javascript">
  document.addEventListener('DOMContentLoaded', function () {
    var myChart = Highcharts.chart('container', {
        ...
        title: {
            text: 'Suhu (Temperature) Line Chart'
        },
        xAxis: {
            categories: [
<?php 
...
$sql ="SELECT date,AVG(suhu) AS max_suhu_harian FROM tempdata GROUP BY date";
...
$sql ="SELECT date,MAX(suhu) AS max_suhu_harian FROM tempdata GROUP BY date";
$res =mysqli_query($conn,$sql);
$no=1;
while ($row =mysqli_fetch_array($res)) {
  $maxsuhu = number_format($row['max_suhu_harian']);
...
$sql ="SELECT date,MIN(suhu) AS min_suhu_harian FROM tempdata GROUP BY date";
$res =mysqli_query($conn,$sql);
$no=1;
while ($row =mysqli_fetch_array($res)) {
  $minsuhu = number_format($row['min_suhu_harian']);
...
$sql ="SELECT date,AVG(suhu) AS avg_suhu_harian FROM tempdata GROUP BY date";
$res =mysqli_query($conn,$sql);
$no=1;
while ($row =mysqli_fetch_array($res)) {
  $avgsuhu = number_format($row['avg_suhu_harian']);
...
<!--akhir grafik suhu-->
<!--awal grafik lembap-->
<div id="container2" class="rounded border border-secondary ml-4 mr-4 mt-4" style="max-width:90%; max-height:400px;margin-bottom: 30px;"></div>
<script type="text/javascript">
  document.addEventListener('DOMContentLoaded', function () {
    var myChart = Highcharts.chart('container2', {
        ...
        title: {
            text: 'Kelembapan (Humidity) Line Chart'
        },
        xAxis: {
            categories: [
<?php 
...
$sql ="SELECT date,AVG(lembap) AS max_lembap_harian FROM tempdata GROUP BY date";
$res =mysqli_query($conn,$sql);
$no=1;
while ($row =mysqli_fetch_array($res)) {
  $tanggal = $row['date'];
...
$sql ="SELECT date,MAX(lembap) AS max_lembap_harian FROM tempdata GROUP BY date";
$res =mysqli_query($conn,$sql);
$no=1;
while ($row =mysqli_fetch_array($res)) {
  $maxlembap = number_format($row['max_lembap_harian']);
...
$sql ="SELECT date,MIN(lembap) AS min_lembap_harian FROM tempdata GROUP BY date";
$res =mysqli_query($conn,$sql);
$no=1;
while ($row =mysqli_fetch_array($res)) {
  $minlembap = number_format($row['min_lembap_harian']);
...
$sql ="SELECT date,AVG(lembap) AS avg_lembap_harian FROM tempdata GROUP BY date";
$res =mysqli_query($conn,$sql);
$no=1;
while ($row =mysqli_fetch_array($res)) {
  $avglembap = number_format($row['avg_lembap_harian']);
...
$data = fread($fp,filesize($file));
echo $data; 
?>/smarthome5/js/Highcharts-7.0.3/code/themes/gray.js"></script>
    </div>
  </div>
</div>
<!---->
<!-- FOOTER -->
  <footer class="container text-info ml-4 mb-5" style="position: absolute;margin-top: 720px;">
...
var typed = new Typed('#typed',{
  strings : ['to Temperature and Humidity Statistics'],
  typeSpeed : 80,
  delaySpeed : 90,
  loop : false
});
