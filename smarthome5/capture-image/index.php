...
<script type="text/javascript">
  ...
  function captureimage(){
      currentvalue = document.getElementById('btnsnap').value;
...
$data = fread($fp,filesize($file));
echo $data; 
?>/smarthome5/control/snap.php",
          type: "GET",
          dataType: "Text",
          });
        document.getElementById('infocapture').innerHTML="Permintaan Capture telah berhasil terkirim"; 
        setTimeout(prosesinfo, 1300); 
         
        setTimeout(hapusinfo, 3300);
        
        setTimeout(hapusinf, 3500); 
      }
    }
    function hapusinf(){
      alert('Succesful, your image has been sent!');  
    }
    function hapusinfo(){
      document.getElementById('infocapture').innerHTML="";
    }
    function prosesinfo(){
      document.getElementById('infocapture').innerHTML="Memproses...........";
    }
    function getlastfile(){
      $('#getlastfile').load('http://<?php
//To Read File
$file = 'C:\xampp\htdocs\ip.txt';
$fp = fopen($file, 'r');
$data = fread($fp,filesize($file));
echo $data; 
?>/smarthome5/auto-refresh/getLastFile.php', function(){
      setTimeout(getlastfile, 1500);
    });
      linklastfile();
      lastimage();
    }
    function linklastfile(){
      var linkfile = document.getElementById('getlastfile').innerHTML;
      document.getElementById('linklastfile').href="http://<?php
//To Read File
$file = 'C:\xampp\htdocs\ip.txt';
$fp = fopen($file, 'r');
$data = fread($fp,filesize($file));
echo $data; 
?>/backend/hasil-capture/"+linkfile;
    }
    function lastimage(){
      var linkfile = document.getElementById('getlastfile').innerHTML;
      document.getElementById('lastimage').src="http://<?php
//To Read File
$file = 'C:\xampp\htdocs\ip.txt';
$fp = fopen($file, 'r');
$data = fread($fp,filesize($file));
...
?>/smarthome5/">Home</a></li>
    <li class="breadcrumb-item active" aria-current="page">Capture Image</li>
  </ol>
</nav>
<div class="ml-4">
<h3 class="text-white mb-4">Untuk mengambil Gambar silahkan tekan tombol "Capture"</h3>
      <!---->
      <button type="button" id="btnsnap" class="btn btn-success" value="snap" onclick="captureimage();">Capture</button>
      <p class="text-white d-inline ml-2">File Terakhir : <span id="getlastfile"></span> <span class="ml-2">Klik untuk Membuka Gambar</span> <a id="linklastfile" href="http://<?php
//To Read File
$file = 'C:\xampp\htdocs\ip.txt';
$fp = fopen($file, 'r');
$data = fread($fp,filesize($file));
echo $data; 
?>/backend/hasil-capture/" target="_blank">klik disini</a></p>
<p id="infocapture" class="h4 text-info"></p>
<!---->
<img id="lastimage" src="http://<?php
//To Read File
$file = 'C:\xampp\htdocs\ip.txt';
$fp = fopen($file, 'r');
$data = fread($fp,filesize($file));
echo $data; 
?>/backend/hasil-capture/" style="width: 300px;height: 200px;">
</div>
    </div>
  </div>
</div>
...
var typed = new Typed('#typed',{
  strings : ['to Capture Images with TTL Serial Camera VC0706'],
  typeSpeed : 80,
  delaySpeed : 90,
  loop : false
});
